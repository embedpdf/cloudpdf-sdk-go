...
```go
response, err := client.Tenants.Create(...)
if err != nil {
...
```go
response, err := client.Tenants.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Tenants.Create(ctx, ...)
```
...
response, err := client.Tenants.Create(ctx, request, ...)
```
